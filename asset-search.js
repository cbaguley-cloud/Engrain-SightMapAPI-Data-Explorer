...
document.addEventListener("DOMContentLoaded", () => {
  // Buttons
  document
    .getElementById("findAssetBtn")
    .addEventListener("click", runSingleAssetSearch);
  document
    .getElementById("assetBulkBtn")
    .addEventListener("click", runBulkAssetSearch);
  document
  ...
  // Template
  document
    .getElementById("assetTemplateLink")
...
async function runBulkAssetSearch() {
  ...
  reader.onload = async function (e) {
    ...
    if (
      lines[0].toLowerCase().includes("asset_name") ||
      lines[0].toLowerCase().includes("property")
    ) {
      lines.shift();
    }
...
function tokenMatchScore(a, b) {
  const t1 = new Set(normalize(a).split(" "));
  const t2 = new Set(normalize(b).split(" "));
  const inter = [...t1].filter((t) => t2.has(t));
  return inter.length / Math.max(t1.size, 1);
}
function combinedScore(name1, name2, city1, city2, state1, state2) {
  const nameScore =
    similarity(name1, name2) * 0.6 + tokenMatchScore(name1, name2) * 0.4;
  const cityScore = similarity(city1, city2);
  const stateScore = state1 && state2 && state1 === state2 ? 1 : 0;
  return Math.min(1, nameScore * 0.7 + cityScore * 0.2 + stateScore * 0.1);
}
...
function scoreColor(score) {
  ...
  if (s > 0) return "background-color:#ef4444;color:white;";
...
function downloadAssetCSV() {
  ...
  let csvContent =
    "Input Name,Input City,Input State,Match Score,Matched Asset ID,Matched Asset Name,Matched Location\n";
