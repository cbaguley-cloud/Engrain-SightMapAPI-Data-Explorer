...
document.addEventListener("DOMContentLoaded", () => {
  // Buttons
  document
    .getElementById("fetchAcctAssetsBtn")
    .addEventListener("click", runAccountAssetFetch);
  document
    .getElementById("copyAcctAssetJsonBtn")
    .addEventListener("click", exportAcctAssetJSON);
  document
    .getElementById("downloadAcctAssetCsvBtn")
    .addEventListener("click", downloadAcctAssetCSV);
  ...
  document
    ...
    .addEventListener("change", renderAcctAssetTable);
  document
    .getElementById("acctFilterState")
    .addEventListener("change", renderAcctAssetTable);
  document
    .getElementById("acctFilterTag")
    .addEventListener("change", renderAcctAssetTable);
});
...
function populateAcctFilters(assets) {
  ...
  assets.forEach((asset) => {
    ...
    if (asset.tags && Array.isArray(asset.tags)) {
      asset.tags.forEach((t) => uniqueTags.add(t));
    }
  });
...
function renderAcctAssetTable() {
  ...
  allAcctAssets.forEach((asset) => {
    ...
    const row = `<tr><td style="font-family:monospace; color:var(--accent-light);">${
      asset.id
    }</td><td style="font-weight:600;">${asset.name}</td><td>${
      asset.address_line1 || ""
    }</td><td>${assetCity}</td><td>${assetState}</td><td>${tagsHtml}</td></tr>`;
...
function updateAcctAssetStatus(msg) {
  document.getElementById("acctAssetStatusMsg").textContent = msg;
}

function exportAcctAssetJSON() {
  if (allAcctAssets.length === 0) {
    alert("No data");
    return;
  }
...
function downloadAcctAssetCSV() {
  ...
  allAcctAssets.forEach((a) => {
    ...
    csvContent += `${a.id},"${cleanName}","${cleanAddr}","${
      a.address_city || ""
    }","${a.address_state || ""}","${tagsStr}"\n`;
  });
...
function copyAcctTable() {
  ...
  allAcctAssets.forEach((a) => {
    ...
    text += `${a.id}\t${a.name}\t${a.address_line1 || ""}\t${
      a.address_city || ""
    }\t${a.address_state || ""}\t${tags}\n`;
  });
