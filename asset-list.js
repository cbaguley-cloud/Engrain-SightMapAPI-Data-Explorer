...
document.addEventListener("DOMContentLoaded", () => {
  // Buttons
  document
    .getElementById("fetchListBtn")
    .addEventListener("click", runListAssets);
  document
    .getElementById("copyListJsonBtn")
    .addEventListener("click", exportListJSON);
  document
    .getElementById("downloadListCsvBtn")
    .addEventListener("click", downloadListCSV);
...
function renderListTable(assets) {
  ...
  assets.forEach((asset) => {
    ...
    const row = `<tr><td style="font-family:monospace; color:var(--accent-light);">${
      ...
      asset.address_line1 || ""
    }</td><td>${asset.address_city || ""}</td><td>${
      asset.address_state || ""
    }</td><td>${tagsHtml}</td></tr>`;
...
function updateListStatus(msg) {
  document.getElementById("listStatusMsg").textContent = msg;
}
