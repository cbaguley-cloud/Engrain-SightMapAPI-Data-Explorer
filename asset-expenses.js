...
document.addEventListener("DOMContentLoaded", () => {
  // Buttons
  document
    .getElementById("fetchExpBtn")
    .addEventListener("click", runSingleExpenseSearch);
  document
    .getElementById("bulkExpBtn")
    .addEventListener("click", runBulkExpenseSearch);
  document
    .getElementById("copyExpJsonBtn")
    .addEventListener("click", exportExpJSON);
  document
  ...
  // Filters
  document
    .getElementById("expFilterType")
    .addEventListener("change", renderExpenseTable);
  document
    .getElementById("expFilterCat")
    .addEventListener("change", renderExpenseTable);
  document
    .getElementById("expFilterFreq")
    .addEventListener("change", renderExpenseTable);
...
async function fetchAndProcessExpenses(apiKey, assetId) {
  ...
  try {
    const nameRes = await fetch(
      `https://api.sightmap.com/v1/assets/${assetId}`,
      {
        method: "GET",
    ...
    if (nameRes.ok) {
      ...
      if (nameJson.name) {
        assetName = nameJson.name;
      } else if (nameJson.data && nameJson.data.name) {
        assetName = nameJson.data.name;
      }
    }
  } catch (e) {
  ...
  try {
    while (nextUrl) {
      ...
      data.forEach((item) => {
        ...
        } else if (item.value_type === "range") {
          finalAmount = `$${item.min_amount} - $${item.max_amount}`;
        }
        const matchObj = {
          asset_id: assetId,
...
function renderExpenseTable() {
  ...
  allExpenses.forEach((exp) => {
    ...
    visibleCount++;
    const reqBadge = exp.is_required
      ? `<span class="match-tag" style="background:#b91c1c; font-size:0.65rem;">YES</span>`
      : `<span style="color:#64748b; font-size:0.8rem;">No</span>`;
...
function updateExpStatus(msg) {
  document.getElementById("expStatusMsg").textContent = msg;
}
...
function downloadExpCSV() {
  ...
  let csvContent =
    "asset_id,asset_name,label,type,category,frequency,amount,is_required\n";
...
function copyExpTable() {
  ...
  let text =
    "Asset ID\tAsset Name\tLabel\tType\tCategory\tFrequency\tAmount\tRequired\n";
